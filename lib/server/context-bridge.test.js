...
// ─── WP-S-7 (M7): Extended Variable Model ────────────────────────────────────

describe('WP-S-7 (M7): Extended Variable Attributes', () => {
  const { AccessLevelFlag } = require('node-opcua');
  const { parseAccessLevel } = require('./context-bridge');

  // Capture-only namespace identical to the one above (kept local to keep
  // these tests self-contained).
  function makeNamespaceMock() {
    const captured = [];
    return {
      addVariable(opts) {
        const variable = { _opts: opts, browseName: opts.browseName };
        captured.push(opts);
        return variable;
      },
      getCalls() { return captured; }
    };
  }

  it('parseAccessLevel handles string with pipe separators', () => {
    const flags = parseAccessLevel('CurrentRead|CurrentWrite|HistoryRead', 0);
    assert.strictEqual(
      flags,
      AccessLevelFlag.CurrentRead | AccessLevelFlag.CurrentWrite | AccessLevelFlag.HistoryRead
    );
  });

  it('parseAccessLevel returns fallback for empty / null input', () => {
    assert.strictEqual(parseAccessLevel('', 7), 7);
    assert.strictEqual(parseAccessLevel(null, 7), 7);
    assert.strictEqual(parseAccessLevel(undefined, 7), 7);
  });

  it('parseAccessLevel passes through a numeric bitmap', () => {
    assert.strictEqual(parseAccessLevel(15, 0), 15);
  });

  it('parseAccessLevel ignores unknown flag names', () => {
    const flags = parseAccessLevel('CurrentRead|Bogus', 0); // TEST DATA
    assert.strictEqual(flags, AccessLevelFlag.CurrentRead);
  });

  it('forwards configured accessLevel to addVariable', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { get: () => 0, set: () => {} };
    createVariableBinding(ns, {}, {
      browseName: 'V', dataType: 'Double', contextKey: 'k',
      accessLevel: 'CurrentRead'
    }, flowCtx, null);
    const opts = ns.getCalls()[0];
    assert.strictEqual(opts.accessLevel, AccessLevelFlag.CurrentRead);
    // userAccessLevel defaults to accessLevel when not set
    assert.strictEqual(opts.userAccessLevel, AccessLevelFlag.CurrentRead);
  });

  it('historizing forces HistoryRead bit and historizing=true', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { get: () => 0, set: () => {} };
    createVariableBinding(ns, {}, {
      browseName: 'V', dataType: 'Double', contextKey: 'k',
    ...
    const opts = ns.getCalls()[0];
    assert.strictEqual(opts.historizing, true);
    assert.ok((opts.accessLevel & AccessLevelFlag.HistoryRead) !== 0);
  });

  it('forwards description, valueRank and arrayDimensions', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { get: () => 0, set: () => {} };
    createVariableBinding(ns, {}, {
      browseName: 'Arr', dataType: 'Double', contextKey: 'arr',
      description: 'An array of doubles', // TEST DATA
      valueRank: 1,
      arrayDimensions: [10] // TEST DATA
    }, flowCtx, null);
    const opts = ns.getCalls()[0];
    assert.strictEqual(opts.description, 'An array of doubles'); // TEST DATA
    assert.strictEqual(opts.valueRank, 1);
    assert.deepStrictEqual(opts.arrayDimensions, [10]); // TEST DATA
  });

  it('setter returns BadUserAccessDenied when CurrentWrite is removed', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { _store: {}, get(k){return this._store[k];}, set(k,v){this._store[k]=v;} };
    createVariableBinding(ns, {}, {
      browseName: 'RO', dataType: 'Double', contextKey: 'ro',
      accessLevel: 'CurrentRead' // read-only
    }, flowCtx, null);
    const opts = ns.getCalls()[0];
    const rc = opts.value.set(new Variant({ dataType: DataType.Double, value: 1 }));
    assert.strictEqual(rc, StatusCodes.BadUserAccessDenied);
    assert.strictEqual(flowCtx._store.ro, undefined);
  });

  it('attaches EURange property when euRange is provided', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { get: () => 0, set: () => {} };
    createVariableBinding(ns, {}, {
      browseName: 'Temp', dataType: 'Double', contextKey: 'temp',
      euRange: { low: 0, high: 100 } // TEST DATA
    }, flowCtx, null);
    const props = ns.getCalls().filter(c => c.browseName === 'EURange');
    assert.strictEqual(props.length, 1, 'EURange property must be created');
    assert.strictEqual(props[0].dataType, 'Range');
  });

  it('attaches EngineeringUnits property when engineeringUnits is provided', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { get: () => 0, set: () => {} };
    createVariableBinding(ns, {}, {
      browseName: 'Temp', dataType: 'Double', contextKey: 'temp',
      engineeringUnits: '°C' // TEST DATA
    }, flowCtx, null);
    const props = ns.getCalls().filter(c => c.browseName === 'EngineeringUnits');
    assert.strictEqual(props.length, 1, 'EngineeringUnits property must be created');
    assert.strictEqual(props[0].dataType, 'EUInformation');
  });

  it('does NOT create EURange when not configured', () => {
    const ns = makeNamespaceMock();
    const flowCtx = { get: () => 0, set: () => {} };
    createVariableBinding(ns, {}, {
      browseName: 'V', dataType: 'Double', contextKey: 'k'
