...
const { buildUserIdentity, reestablishOrCreateSession, reactivateSubscriptions } = require('./session-manager');
...
describe('reactivateSubscriptions()', () => {

  it('returns all subscriptions as active when setPublishingMode succeeds', async () => {
    const sub1 = { setPublishingMode: async () => {} };
    const sub2 = { setPublishingMode: async () => {} };

    const { active, expired } = await reactivateSubscriptions([sub1, sub2], null);
    assert.strictEqual(active.length, 2);
    assert.strictEqual(expired.length, 0);
  });

  it('moves failed subscriptions to expired list', async () => {
    const sub1 = { setPublishingMode: async () => {} };
    const sub2 = { setPublishingMode: async () => { throw new Error('BadSubscriptionIdInvalid'); } };

    const logger = { warn: () => {} };
    const { active, expired } = await reactivateSubscriptions([sub1, sub2], logger);
    assert.strictEqual(active.length, 1);
    assert.strictEqual(expired.length, 1);
    assert.strictEqual(expired[0], sub2);
  });

  it('returns empty arrays for empty input', async () => {
    const { active, expired } = await reactivateSubscriptions([], null);
    assert.strictEqual(active.length, 0);
    assert.strictEqual(expired.length, 0);
  });

  it('logs warning for each failed subscription when logger is provided', async () => {
    const warnings = [];
    const logger = { warn: (msg) => warnings.push(msg) };
    const sub = { setPublishingMode: async () => { throw new Error('Gone'); } };

    await reactivateSubscriptions([sub], logger);
