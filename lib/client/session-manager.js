...
async function reactivateSubscriptions(subscriptions, logger) {
  ...
  const results = await Promise.allSettled(
    subscriptions.map(sub =>
      sub.setPublishingMode(true).then(() => sub)
    )
  );

  for (let i = 0; i < results.length; i++) {
    if (results[i].status === 'fulfilled') {
      active.push(results[i].value);
    } else {
      if (logger) logger.warn(`Subscription reactivation failed: ${results[i].reason?.message} — marked as expired`);
      expired.push(subscriptions[i]);
    }
  }
